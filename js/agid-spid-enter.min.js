function getSpidI18n(labelKey) {
    var locale = document.getElementsByTagName("html")[0].getAttribute("lang") || window.agidSpidEnterI18n["spid-texts"].default, copy = window.agidSpidEnterI18n.language && window.agidSpidEnterI18n.language[locale] && window.agidSpidEnterI18n.language[locale][labelKey];
    return window.agidSpidEnterI18n.language[locale] ? window.agidSpidEnterI18n.language[locale][labelKey] || console.error("La chiave richiesta non è disponibile nel locale selezionato:", labelKey) : console.error("Il locale richiesto non è disponibile:", locale), 
    copy || labelKey;
}
...
window.agidSpidEnterConfig = {
...
}, window.agidSpidEnterI18n = {}, window.agidSpidEnterTpl = {
    ...
    spidButton: function(sizeClass) {
        return [ "\x3c!-- AGID - SPID BUTTON ", sizeClass.toUpperCase(), " * begin * --\x3e", '<button class="agid-spid-enter agid-spid-enter-size-', sizeClass, '" style="display:none">', '<span aria-hidden="true" class="agid-spid-enter-icon">', '<img aria-hidden="true" src="img/spid-ico-circle-bb.svg" onerror="this.src=\'img/spid-ico-circle-bb.png\'; this.onerror=null;" alt="', getSpidI18n("entra_con_SPID"), '" />', "</span>", '<span class="agid-spid-enter-text">', getSpidI18n("entra_con_SPID"), "</span>", "</button>", "\x3c!-- AGID - SPID BUTTON ", sizeClass.toUpperCase(), " * end * --\x3e" ].join("");
    },
    infoModal: function(htmlContent) {
        return [ '<div class="modal-content">', '<span id="closemodalbutton" class="close" role="button" tabindex="0" aria-label="', getSpidI18n("aria_chiudi_modale"), '"><b aria-hidden="true">&times;</b></span>', "<div>", htmlContent, "</div>", "</div>" ].join("");
    },
    nonHaiSpid: function() {
    ...
    function openInfoModal(htmlContent) {
        infoModal.innerHTML = agidSpidEnterTpl.infoModal(htmlContent), infoModal.style.display = "block", 
    ...
    function getLocalisedMessages() {
        return ajaxRequest("GET", agidSpidEnterConfig.spidLocalisationEndpoint).then(function(i18nData) {
    ...
    function getAvailableProviders() {
        return ajaxRequest("GET", agidSpidEnterConfig.spidProvidersEndpoint).catch(function(e) {
    ...
    function bindEscKeyEvent() {
        document.addEventListener("keyup", function(e) {
            var isEscKeyHit = 27 === e.keyCode, isInfoModalVisible = isElementVisible(infoModal), isProvidersPanelVisible = isElementVisible(spidPanelSelect);
    ...
    var infoModal, spidPanelSelect, hasSpidProviders = !1;
    return renderSpidModalContainer(), getLocalisedMessages().then(getAvailableProviders).then(renderAvailableProviders).then(renderSpidButtons).then(getSelectors).then(bindEscKeyEvent), 
    {
