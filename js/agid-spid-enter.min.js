window.AgidSpidEnter = function() {
    var self = this, hasSpidProviders = false, agidSpidEnterWrapper, spidIdpList, infoModal, infoModalContent, spidPanelSelect;
...
window.AgidSpidEnter.prototype.tpl = {
    spidMainContainers: function() {
        return [ '<div id="agid-spid-enter"></div>', '<div id="agid-infomodal" class="modal" aria-live="assertive" hidden>', '<div class="modal-content agid-font">', '<a id="closemodalbutton"', 'class="close agid-navigable"', 'role="button"', 'tabindex="0"', 'aria-label="', this.getI18n("aria_chiudi_modale"), '" href="#">', '<b aria-hidden="true">&times;</b>', "</a>", '<div id="agid-infomodal-content" role="main" tabindex="0" aria-labelledby="agid-infomodal-title">', "</div>", "</div>", "</div>" ].join("");
    },
    ...
    spidProviderButton: function(providerData) {
        var providerPayload, title = providerData.isActive ? this.getI18n("accedi_con_idp", providerData.title) : this.getI18n("idp_disabled");
        providerPayload = this.tpl.hiddenField("provider", providerData.provider);
        return [ '<span class="agid-spid-col l3 m6 s6 xs12">', '<form action="', this.formActionUrl, '" method="', this.formSubmitMethod, '">', '<button type="submit"', 'class="agid-spid-idp-button agid-spid-idp-', providerData.provider, '"', 'title="', title, '"', 'style="background-image: url(', this.config.assetsBaseUrl, "img/idp-logos/", providerData.logo, ')"', providerData.isActive ? "" : "disabled", ">", "</button>", providerPayload, "</form>", "</span>" ].join("");
    },
